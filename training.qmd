...
We are a team of computational biologists, data scientists, bioinformatics software developers, and more! Our interests range from human genomics to structural biology and the microbiome.
...
You can learn more on our website: [bioinformatics.niaid.nih.gov](https://bioinformatics.niaid.nih.gov/). You will find information about future trainings, can view previous trainings, and start a new collaboration!
...
- I'm sharing the approaches I typically take for analysis and know very well. Throughout the tutorial, I'll point to alternative methods that are available. There are still active debates about which methods are better. However, I hope that the ideas I share today will at least provide you with avenues of exploration and a platform upon which you can build. And we're always available for a consult!

- Trouble installing some of the packages? I've created an Rstudio instance in the cloud that includes preinstallations of all of the packages we'll be using today. The code you run and edits you make won't be saved, but it will let you play around with the data we have today.
...
Most of the analysis we do today will use the Amplicon Metagenomics (DADA2) pipeline results, but the concepts to analyze Shotgun Metagenomics (WGSA2) pipeline results are highly similar.

# Making a Phyloseq Object

[Phyloseq](https://joey711.github.io/phyloseq/) is an R package that allows for relatively simple exploration of microbiome data and keeps the key components you would need all in one easily accessible object^[McMurdie PJ, Holmes S. Phyloseq: a bioconductor package for handling and analysis of high-throughput phylogenetic sequence data. Pac Symp Biocomput. 2012:235-46. PMID: 22174279; PMCID: PMC3357092.]. There are then different and useful accessors and processors that we will get to play around with more today. This diagram gives a detailed view of how each of the components fit together.

![Phyloseq Overview](images/phyloseq_overview.png)

## DADA2

The data that we will be using today is derived from a paper examining differences in the skin microbiome of infants in the United States and Mexico^[https://pubmed.ncbi.nlm.nih.gov/33144313/]. I have included the citation and abstract (with relevant highlighting) below.
...
```{r}
#| fig-width: 9
dada2_class <- tax_glom(dada2_phyloseq_object, "Family", NArm = F)
plot_bar(dada2_class, fill="Family") + guides(fill="none") + scale_fill_viridis_d()
```

::: {.callout-tip}
## Question

Tell me three things about the data based on this plot.
:::

Another type of figure I like to do very early (and we'll talk about it more later) is a Principal Coordinates Analysis (PCoA) plot. This plot is useful because it puts samples in relationship to each other. In these plots, samples are points and closer points represent more similar samples while further points represent further samples.
...
sort(sample_sums(dada2_phyloseq_filtered))
```

In this more complex line of code, I'm taking a look at the sample data associated with samples having fewer than 1000 reads -- I wanted to see if there was anything special noted in the sample data that might help us understand why there are so few reads.

```{r}
sample_data(dada2_phyloseq_filtered)[sample_sums(dada2_phyloseq_filtered) < 1000]
```

Overall, we can see a wide variety of sequencing depth across samples. Some samples have very few reads, while others have several thousand. We typically want to "standardize" this in some way because differences in sequencing effort can contribute to potential bias. How this happens, though, is an area of active, lively debate that continues to this day. And new methods are being developed, though I haven't seen uptake of the new methods changing very much.

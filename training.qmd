...
::: {.callout-tip}
I'll show you how I typically approach network analysis using SparCC and igraph. Unfortunately, my laptop doesn't allow the installation of the SpiecEasi package (which includes SparCC), so I'll need to show you this step through the Binder included with the github repository. You can access the new container with a click at the top of the html.

The code that I use is printed below, it just won't evaluate to anything.
:::

Here, I'm going to ask a slightly different question. Instead of separating out sample types, I am going to bring all of the samples back together and perform network analysis on the whole set of samples. The intention is to determine whether there are differential network structures between sample types.
...
cor_net <- SpeicEasi::sparcc(otu_table(dada2_rare))
```

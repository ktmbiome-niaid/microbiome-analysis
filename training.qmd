...
I processed the raw data with Nephele using the parameters the authors provided in their manuscript. However, the tables we analyze today are not identical to the results presented in the manuscript.
...
We next need to separate our taxonomy information from this table so that it can be analyzed separately.
...
Let's first refresh some basic statistics concepts that I'm going to be mentioning throughout today and make sure everyone's on the same page.

**P-value**: The probability that the relationship we are seeing is due to chance.

**Type I error/"alpha"**: The level of false-positives that we are willing to accept. This is typically set to ~5% or 0.05, and indicates that about 5% of the time, a significant result is in fact not significant.
...
The first most common data distribution we will encounter several times today is "normal" or "Gaussian". When a variable is normally-distributed, it typically takes on a bell-shaped curve that is centered around a mean.

The others that will be important later are:
...
![Statistical Test "Cheat Sheet"](images/StatisticalTestCheatSheet.png)
...
Hopefully the workflow can serve as a general guide for which tests you might want to use. We will be unable to cover everything today, but perhaps some of the other test types can serve as bread crumbs for future exploration. We will also cover a few tests beyond those covered in the above figure for some of the multi-dimensional statistics we use in microbiome data.
...
One of the important things to note about this step of analysis is that we're likely to be analyzing counts, which follow their own unique set of distributions that are typically not Gaussian/normally-distributed, and thus we'll start to dabble in some of the other distributions I mentioned near the beginning.

In some methods, you might **not** analyze actual count data. This is because additional transformations that pull the taxa into more Gaussian-like distributions are preferred.

:::{callout-tip}
All of these discussions about count data are **super** important for 16s rRNA count data, but are often less important for metagenomic data. It's still somewhat sparse, but starts approaching some semblance of approximately normal, especially after log-transformation, allowing more gaussian models to be used.
:::

For now, I will show you how I use one particularly flexible tool called Maaslin2 which can be used to analyze the data we're working with today. I'm still interested in the comparison of countries, and want to know what specific taxa are enriched in the forehead samples in the USA vs Mexico. 
...
capture.output(
  run_maaslin <- Maaslin2(input_data= data.frame(otu_table(dada2_rare_ha)),
           ...
           min_prevalence = 0.4, # a taxon needs to be in 40% of my samples to be considered/analyzed
           min_variance = 0.1 #I also like having some variance restrictions -- taxa that don't vary much across samples can cause trouble
...
Maaslin kindly produces some boxplots of significant taxa that are accessible within the output directory. I tend to use them for quick review when I need them, and we can make a few boxplots ourselves here.
...
Finally, I want to focus on something kind of "fun" and "different" for the end of our day. We know that bacteria often do not live on or in the human body in isolation but rather as a broader collaborative network with several other species and genera. We can therefore start to use that knowledge to ask about "cliques" or "networks" of co-occurring taxa and whether abundances of these broader networks provides more insight about the differences that we're interested in.

As has been true for almost everything we've discussed so far, there are several methods that exist to perform network analysis. These methods can include:

- *Correlation*: Use simple correlations of each taxon to all other taxa to determine which combinations reach some threshold (either correlation coefficient or p-value). Ideally, you'd want to use Spearman correlation over Pearson correlation for this because it is a non-parametric test.

- *Distance Matrix*: Calculate distances between taxa or samples using a preferred distance calculation. This is the method that phyloseq utilizes by default.

- *SparCC*^[Friedman J, Alm EJ. Inferring correlation networks from genomic survey data. PLoS Comput Biol. 2012;8(9):e1002687. doi: 10.1371/journal.pcbi.1002687. Epub 2012 Sep 20. PMID: 23028285; PMCID: PMC3447976.]: Correlation using sparsity and compositionality corrections because, as we've discussed before, microbiome data is sparse which makes correlations less reliable.

- *Correlation inference for Compositional data through LASSO (CCLasso)*^[Fang H, Huang C, Zhao H, Deng M. CCLasso: correlation inference for compositional data through Lasso. Bioinformatics. 2015 Oct 1;31(19):3172-80. doi: 10.1093/bioinformatics/btv349. Epub 2015 Jun 4. PMID: 26048598; PMCID: PMC4693003.]: This 

- *Weighted Gene Co-Association Network Analysis (WGCNA)*^[Langfelder P, Horvath S. WGCNA: an R package for weighted correlation network analysis. BMC Bioinformatics. 2008 Dec 29;9:559. doi: 10.1186/1471-2105-9-559. PMID: 19114008; PMCID: PMC2631488.]: This method is distance-based with additional corrections. Its goal is to find clusters of highly correlated features in data and summarizes them using an "eigengene". This tool tends to be better for metagenomic data than amplicon data.

If you want to read more about different network methods, I have always found the following paper to be helpful:

> Layeghifard M, Hwang DM, Guttman DS. Disentangling Interactions in the Microbiome: A Network Perspective. Trends Microbiol. 2017 Mar;25(3):217-228. doi: 10.1016/j.tim.2016.11.008. Epub 2016 Dec 2. PMID: 27916383; PMCID: PMC7172547.

While I won't cover it today, SparCC also offers the option to provide bootstrapped estimates and p-values. This is done in a similar manner to how PERMANOVA p-values are calculated: the correlation coefficients are re-calculated based on resampling the data with replacement and then creates p-values by determining the proportion of times the "random" value is greater than the original value.
...
dada2_rare_filt <- filter_taxa(dada2_rare_ha, function(x) (sum(x > 0)/length(x)) > 0.1, TRUE)
dada2_rare_filt
## These are the commands I used to create the sparCC matrix -- note that I have transposed the OTU table so that 
# cor_df <- sparcc(t(otu_table(dada2_rare_filt)))
# saveRDS(cor_df, "Sparcc_CorrMatrix.rds")
cor_df <- readRDS("Sparcc_CorrMatrix.rds")
str(cor_df)
rownames(cor_df$Cor) <- colnames(cor_df$Cor) <- taxa_names(dada2_rare_filt)

net_dist <- 1-abs(cor_df$Cor)
```

Now that we have our network of correlation coefficients, we can use some phyloseq functions to plot the result.

```{r}
#| fig-width: 10
#| fig-height: 12
ig <- make_network(dada2_rare_filt, type="taxa", distance=as.dist(net_dist), max.dist=0.6)

plot_network(ig, dada2_rare_filt, type="taxa", color="Order") + scale_color_viridis_d()
```
